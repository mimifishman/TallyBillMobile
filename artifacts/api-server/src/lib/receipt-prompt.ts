...
- A line is NOT a modifier if its amount is NEGATIVE, or if its wording names a discount — "Happy Hour", "promotion", "remise", "loyalty", "member", "הנחה", or any percentage off. Those are discounts however deeply they are indented, and dropping one as though it were a modifier overcharges the diners. Where it goes depends only on WHERE IT IS PRINTED: indented under an item it belongs to that item under CASE 1, and printed after the items subtotal it belongs to the whole bill under CASE 3, never to the item that happens to be above it.
